...
def dashboard(
    ...
    # Keep backwards compatibility for older desktop clients: if the preferred
    # currency has no income, expose the first real income currency instead of
    # reporting a misleading zero. The complete per-currency map is still returned.
    revenue_currency = display_currency if display_currency in revenue_by_currency else (sorted(revenue_by_currency)[0] if revenue_by_currency else display_currency)
    expected_currency = display_currency if display_currency in expected_by_currency else (sorted(expected_by_currency)[0] if expected_by_currency else display_currency)
    revenue = revenue_by_currency.get(revenue_currency, Decimal("0"))
    expected_revenue = expected_by_currency.get(expected_currency, Decimal("0"))
    ...
    by_type = {}
    revenue_by_format = {}
    counted_sales_by_type = set()
    for split, sale in paid_split_rows:
        if split.role not in {"producer", "messenger"}:
            continue
        sale_type_key = (sale.id, sale.license_type)
        if sale_type_key not in counted_sales_by_type:
            by_type[sale.license_type] = by_type.get(sale.license_type, 0) + 1
            counted_sales_by_type.add(sale_type_key)
        currency = sale.currency
        revenue_by_format.setdefault(currency, {})
        revenue_by_format[currency][sale.license_type] = (
            revenue_by_format[currency].get(sale.license_type, Decimal("0"))
    ...
    if prev_start is not None:
        prev_stmt = (
            select(LicenseSplit.amount)
            .join(License, License.id == LicenseSplit.license_id)
            .where(
                LicenseSplit.user_id == current_user.id,
                License.status == 'paid',
                License.currency == revenue_currency,
                License.purchased_at >= prev_start,
            )
        )
        if prev_end is not None:
            prev_stmt = prev_stmt.where(License.purchased_at < prev_end)
        prev_revenue = sum((Decimal(str(x or 0)) for x in db.scalars(prev_stmt).all()), Decimal('0'))
        if prev_revenue != 0:
    ...
    top_artist_stmt = (
        select(License.artist_id, func.count(func.distinct(License.id)), func.sum(LicenseSplit.amount))
        .join(License, License.id == LicenseSplit.license_id)
        .where(
            LicenseSplit.user_id == current_user.id,
            License.status == 'paid',
            LicenseSplit.role.in_(['producer', 'messenger']),
            License.currency == revenue_currency,
        )
    )
    if start is not None:
    ...
    top_artist_rows = db.execute(
        top_artist_stmt
        .group_by(License.artist_id)
        .order_by(func.sum(LicenseSplit.amount).desc())
        .limit(5)
    ).all()
    top_artists=[]
    for artist_id, sale_count, artist_revenue in top_artist_rows:
        artist=db.get(Artist, artist_id)
        top_artists.append({'artist_id':artist_id,'name':artist.name if artist else f'Artist #{artist_id}','sales':int(sale_count or 0),'revenue':str(artist_revenue or 0),'currency':display_currency})

    return {
        "period": period,
        "display_currency": display_currency,
        "revenue_currency": revenue_currency,
        "expected_revenue_currency": expected_currency,
        "revenue": str(revenue),
        ...
        "revenue_by_currency": {c: str(v) for c,v in sorted(revenue_by_currency.items())},
        "expected_revenue_by_currency": {c: str(v) for c,v in sorted(expected_by_currency.items())},
        "balance_by_currency": {c: str(v) for c,v in sorted(revenue_by_currency.items())},

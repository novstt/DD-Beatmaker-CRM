...
def time_greeting(name: str) -> str:
    ...
    return f"{greeting}, {canonical_producer_label(name or 'Producer')} 👋"


def time_greeting_plain(name: str) -> str:
    hour = datetime.now().hour
    if 5 <= hour < 12:
        greeting = "Good morning"
    elif 12 <= hour < 18:
...
def _update_manifest_url():
    env = os.environ.get("DD_UPDATE_MANIFEST_URL", "").strip()
    if env:
        return env
    try:
...
def _install_root():
    """Return the directory containing DD.exe in both frozen and source runs."""
...
def _pending_update_path():
    return Path(_prefs_path()).parent / "pending_update.json"

def _write_pending_update(version, notes):
    try:
        path = _pending_update_path()
        path.write_text(json.dumps({"version": version, "notes": notes or []}, ensure_ascii=False, indent=2), encoding="utf-8")
    except Exception:
        pass

def _take_pending_update():
    path = _pending_update_path()
    try:
        if not path.exists():
            return None
...
class PreviewDashboard(QWidget):
    ...
    def __init__(self):
        ...
        title = QLabel(time_greeting_plain("SLV"))
...
class StatsTab(QWidget):
    ...
    def __init__(self, api):
        ...
        self.role_breakdown=QLabel("Producer — 0.00   •   Messenger — 0.00"); self.role_breakdown.setObjectName("mutedLabel"); rv.addWidget(self.role_breakdown)
    ...
    def refresh(self):
        try:
            ...
            preferred_cur=(self.api.user or {}).get("currency","USD").upper()
            balances=data.get("balance_by_currency") or data.get("revenue_by_currency") or {}
            revenue_cur=data.get("revenue_currency") or preferred_cur
            if preferred_cur in balances:
                revenue_cur=preferred_cur
            elif balances:
                revenue_cur=sorted(balances)[0]
            revenue=float(balances.get(revenue_cur, data.get("revenue",0)) or 0)
            expected_map=data.get("expected_revenue_by_currency") or {}
            expected_cur=preferred_cur if preferred_cur in expected_map else (sorted(expected_map)[0] if expected_map else data.get("expected_revenue_currency") or preferred_cur)
            expected=float(expected_map.get(expected_cur, data.get("expected_revenue",0)) or 0)
            ...
            self.kpi_revenue.update_value(f"{revenue_cur} {revenue:,.2f}", delta_text)
            self.kpi_pending.update_value(f"{expected_cur} {expected:,.2f}", f"Pending licenses: {data.get('pending_licenses',0)}")
            ...
            self.revenue_value.setText(f"{revenue_cur} {revenue:,.2f}")
            self.revenue_delta.setText(delta_text)
            producer_map = data.get("producer_earnings_by_currency") or {}
            messenger_map = data.get("messenger_earnings_by_currency") or {}
            producer_val = float(producer_map.get(revenue_cur, 0) or 0)
            messenger_val = float(messenger_map.get(revenue_cur, 0) or 0)
            self.role_breakdown.setText(f"Producer  {revenue_cur} {producer_val:,.2f}   •   Messenger  {revenue_cur} {messenger_val:,.2f}")
...
class HomeTab(QWidget):
    ...
    def __init__(self, api):
        ...
        self.greeting = QLabel(time_greeting((api.user or {}).get("username", "SLV")))
    ...
    def _update_greeting(self):
        self.greeting.setText(time_greeting((self.api.user or {}).get("username", "SLV")))
    ...
    def refresh(self):
        try:
            ...
            total_cur=all_data.get("revenue_currency") or cur
            month_cur=data.get("revenue_currency") or cur
            total=all_data.get("revenue",0); month=data.get("revenue",0)
            self.total_revenue.update_value(self._money(total,total_cur),"All time")
            delta=data.get("revenue_change_percent")
            delta_text=(f"{float(delta):+.0f}% vs last month" if delta is not None else "This month")
            self.month_revenue.update_value(self._money(month,month_cur),delta_text)
...
class WhatsNewDialog(QDialog):
    def __init__(self, version, notes, parent=None):
        ...
        head=QHBoxLayout(); icon=QLabel("🎉"); icon.setStyleSheet("font-size:30px;"); head.addWidget(icon)
        titles=QVBoxLayout(); t=QLabel("What's New"); t.setStyleSheet("font-size:28px;font-weight:800;"); titles.addWidget(t)
        st=QLabel(f"D&D has been updated to version {version}"); st.setStyleSheet("color:#9BA3B3;font-size:14px;"); titles.addWidget(st)
        head.addLayout(titles,1); close=QPushButton("×"); close.setObjectName("iconButton"); close.clicked.connect(self.reject); head.addWidget(close); root.addLayout(head)
        line=QFrame(); line.setFrameShape(QFrame.Shape.HLine); line.setStyleSheet("color:#262C39;"); root.addWidget(line)
        content=QHBoxLayout();
        left=QFrame(); left.setObjectName("sectionCard"); lv=QVBoxLayout(left); lv.setContentsMargins(24,24,24,24)
        logo=QLabel("D&D"); logo.setAlignment(Qt.AlignmentFlag.AlignCenter); logo.setStyleSheet("font-size:42px;font-weight:900;color:#A35CFF;background:#17111F;border:1px solid #5A327E;border-radius:22px;padding:24px;"); lv.addWidget(logo)
        vt=QLabel(f"Version {version}"); vt.setAlignment(Qt.AlignmentFlag.AlignCenter); vt.setStyleSheet("font-size:22px;font-weight:800;margin-top:12px;"); lv.addWidget(vt)
        ok=QLabel("✓  Update successful"); ok.setAlignment(Qt.AlignmentFlag.AlignCenter); ok.setStyleSheet("color:#35D07F;background:#10241B;border:1px solid #1D5D3E;border-radius:10px;padding:10px;margin-top:12px;"); lv.addWidget(ok); lv.addStretch(); content.addWidget(left,1)
        right=QFrame(); right.setObjectName("sectionCard"); rv=QVBoxLayout(right); rv.setContentsMargins(24,22,24,22)
        title=QLabel("What’s new in this update"); title.setStyleSheet("font-size:18px;font-weight:800;color:#C18BFF;"); rv.addWidget(title)
        scroll=QScrollArea(); scroll.setWidgetResizable(True); scroll.setFrameShape(QFrame.Shape.NoFrame)
        inner=QWidget(); iv=QVBoxLayout(inner); iv.setContentsMargins(2,2,8,2); iv.setSpacing(10)
        groups={"Finance & Payments":[],"Statistics":[],"User Experience":[]}
        for n in notes or []:
            text=str(n)
            if ":" in text:
                g, item=text.split(":",1); g=g.strip(); item=item.strip()
                groups.setdefault(g,[]).append(item)
            else: groups["User Experience"].append(text)
        for g,items in groups.items():
            if not items: continue
            gl=QLabel(g); gl.setStyleSheet("font-size:15px;font-weight:800;"); iv.addWidget(gl)
            for item in items:
                bl=QLabel("•  "+item); bl.setWordWrap(True); bl.setStyleSheet("color:#D4D8E0;font-size:13px;"); iv.addWidget(bl)
            sep=QFrame(); sep.setFrameShape(QFrame.Shape.HLine); sep.setStyleSheet("color:#262C39;"); iv.addWidget(sep)
        thanks=QLabel("🚀  Thank you for using D&D!\nWe’re constantly improving the platform to make it better for you."); thanks.setWordWrap(True); thanks.setStyleSheet("color:#CFC5DE;background:#191329;border:1px solid #382650;border-radius:12px;padding:14px;font-size:13px;"); iv.addWidget(thanks)
        iv.addStretch(); scroll.setWidget(inner); rv.addWidget(scroll,1); content.addWidget(right,2); root.addLayout(content,1)
        foot=QHBoxLayout(); foot.addStretch(); b=QPushButton("Continue to D&D"); b.setObjectName("primary"); b.setMinimumSize(190,44); b.clicked.connect(self.accept); foot.addWidget(b); root.addLayout(foot)
...
class SettingsTab(QWidget):
    ...
    def _fetch_update_manifest(self):
        ...
        if not url:
            # Fall back to the backend version endpoint for legacy setups.
            data=self.api.app_version()
            latest=str(data.get("latest_version") or data.get("version") or APP_VERSION)
            return {"version":latest,"package_url":data.get("package_url") or data.get("update_url"),"sha256":data.get("sha256"),"notes":data.get("notes",[])}
    ...
    def _launch_updater(self, manifest_url, package_url, sha256):
        ...
        if updater_exe.exists():
            ...
            else: args += ["--package-url",package_url or "", "--sha256",sha256 or ""]
            subprocess.Popen(args, cwd=str(install_dir), close_fds=True)
            self.close(); return True
        updater_py=install_dir/"updater"/"updater.py"
        if updater_py.exists() and not getattr(sys,"frozen",False):
            args=[sys.executable,str(updater_py),"--install-dir",str(install_dir),"--exe","DD.exe","--parent-pid",str(os.getpid())]
            if manifest_url: args += ["--manifest",manifest_url]
            else: args += ["--package-url",package_url or "", "--sha256",sha256 or ""]
            subprocess.Popen(args, cwd=str(install_dir)); self.close(); return True
        raise FileNotFoundError("D&D Updater is not installed with this build.")

    def check_updates(self, silent=False):
        try:
            data=self._fetch_update_manifest(); version=str(data.get("version",APP_VERSION)); available=_version_tuple(version)>_version_tuple(APP_VERSION)
            if not available:
                if not silent: QMessageBox.information(self,'D&D Update',f'You are running D&D {APP_VERSION}.\n\nNo update is available.')
                return False
            notes=data.get("notes") or []
            url=_update_manifest_url()
            package_url=data.get("package_url"); sha=data.get("sha256")
            box=QMessageBox(self); box.setWindowTitle('D&D Update'); box.setText(f'New D&D version {version} is available.'); box.setInformativeText('The update package will be verified before installation. Your data stays on the server.')
            ...
            if box.clickedButton() is update_btn:
                if not url and not package_url: raise ValueError("No update package is configured.")
                return self._launch_updater(url,package_url,sha)
            return False
        except Exception as e:
            if not silent: QMessageBox.warning(self,'D&D Update',str(e))
            return False
...
class AccountPanel(QFrame):
    ...
    def refresh_balance(self):
        try:
            ...
            preferred=(self.api.user or {}).get("currency","USD").upper()
            balances=data.get("balance_by_currency") or data.get("revenue_by_currency") or {}
            currency=preferred if preferred in balances else (sorted(balances)[0] if balances else data.get("revenue_currency") or preferred)
            value=float(balances.get(currency, data.get("revenue",0)) or 0)
...
class MainWindow(QMainWindow):
    ...
    def _show_pending_update_notes(self):
        ...
        QTimer.singleShot(500, lambda: WhatsNewDialog(str(pending.get("version") or APP_VERSION), pending.get("notes") or [], self).exec())
